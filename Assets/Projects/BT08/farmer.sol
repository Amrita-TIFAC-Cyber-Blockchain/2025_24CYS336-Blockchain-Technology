...
contract DeFiForFarmsSimple {
    ...
    nextBatch++;
    batches[nextBatch] = Batch(nextBatch, msg.sender, prod, qty, new string[](0), true);
    ...
    function applyLoan(uint amt) external {
        require(farmers[msg.sender].exists, "not farmer");
        nextLoan++;
        loans[nextLoan] = Loan(nextLoan, msg.sender, address(0), amt, 0, false);
    }
    ...
    function disburseLoan(uint id) external payable onlyLender nonReentrant {
        ...
        require(msg.value == L.amount, "send full loan");
