...
contract DefenceLogistics {
    ...
    struct Asset {
        ...
        string ipfsHash; // Doc or certificate pointer
        StatusEvent[] history; // Full audit trail
    }

    mapping(uint256 => Asset) public assetRecords;

    // Role addresses (replace with real MetaMask Sepolia testnet accounts for each)
    address public logisticsOfficer;
    address public depotManager;
    address public agencyAuditor;
    address public owner;

    // Events for tracking (event logging)
    ...
    constructor() {
        // Set these to your real MetaMask account addresses
        owner = msg.sender;
    ...
    // Asset Registration (by logistics officer)
    ...
    // Ownership/Holder Transfer (by depot manager)
    ...
    // Change Supplier (by logistics officer, reflect in event log)
